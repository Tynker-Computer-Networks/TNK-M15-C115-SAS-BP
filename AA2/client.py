...
def move_player(steps, color):
    ...
    # Create a start_index variable with value None
    
    for box in boxes[::-1]:
        if(box.cget("bg") == color):
            if steps+boxes.index(box) <= len(boxes):
                print("new index:", steps+boxes.index(box), "old index", boxes.index(box))
                if(boxes.index(box) != 0):
                    box.configure(bg='white')
                steps += boxes.index(box)
                # Set start_index to index of the current box
    ...
    if(updated):
        # Run a for each loop on boxes list from index start_index to steps+1
